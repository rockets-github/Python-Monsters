...
class AllSprites(pygame.sprite.Group):
    def __init__(self):
        ...
        self.shadow_surf = import_image(
            Path.joinpath(Path(__file__).parent.parent, "graphics", "other", "shadow")
        )
        self.notice_surf = import_image(
            Path.joinpath(Path(__file__).parent.parent, "graphics", "ui", "notice")
        )
        ...
        for layer in (bg_sprites, main_sprites, fg_sprite):
            for sprite in layer:
                ...
                if sprite == player and player.noticed:
                    ...
                    self.display_surface.blit(
                        self.notice_surf, rect.topleft + self.offset
                    )
...
class BattleSprites(pygame.sprite.Group):
    ...
    def draw(
        ...
        sprite_group = opponent_sprite if side == "opponent" else player_sprites
        ...
        for sprite in sorted(self, key=lambda sprite: sprite.z):
            if sprite.z == BATTLE_LAYERS["outline"]:
                if (
                    ...
                    or sprite.monster_sprite == monster_sprite

...
class MonsterPatchSprite(Sprite):
    def __init__(self, pos, surf, groups, biome):
        self.biome = biome
        super().__init__(
            pos, surf, groups, z=WORLD_LAYERS["main" if biome != "sand" else "bg"]
        )
        self.y_sort -= 40
...
class MonsterOutlineSprite(pygame.sprite.Sprite):
    def __init__(self, monster_sprite, groups, frames):
        ...
        self.image = self.frames[self.monster_sprite.state][
            self.monster_sprite.frame_index
        ]
        self.rect = self.image.get_frect(center=self.monster_sprite.rect.center)

    def update(self, _):
        self.image = self.frames[self.monster_sprite.state][
            self.monster_sprite.adjusted_frame_index
        ]
...
class MonsterNameSprite(pygame.sprite.Sprite):
    def __init__(self, pos, monster_sprite, groups, font):
        ...
        text_surf = font.render(
            self.monster_sprite.monster.name, False, COLORS["black"]
        )
        padding = 10
...
class MonsterStatsSprite(pygame.sprite.Sprite):
    ...
    def update(self, _):
        ...
        for index, (value, max_value) in enumerate(
            ...
            if index < 2:
                text_surf = self.font.render(
                    f"{int(value)}/{max_value}", False, COLORS["black"]
                )
                text_rect = text_surf.get_frect(
                    topleft=(self.rect.width * 0.05, index * self.rect.height / 2)
                )
                bar_rect = pygame.FRect(
                    text_rect.bottomleft + vector(0, -2), (self.rect.width * 0.9, 4)
                )

                self.image.blit(text_surf, text_rect)
                draw_bar(
                    self.image, bar_rect, value, max_value, color, COLORS["black"], 2
                )
            else:  # initiative
                init_rect = pygame.FRect(
                    (0, self.rect.height - 2), (self.rect.width, 2)
                )
                draw_bar(
                    self.image, init_rect, value, max_value, color, COLORS["white"], 0
                )
        if not self.monster_sprite.groups():
            self.kill()

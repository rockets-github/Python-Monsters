...
class MonsterIndex:
            ...
            for stat, value in data["stats"].items():
                if stat != "element":
                    if stat not in self.max_stats:
                        ...
                        self.max_stats[stat] = max(value, self.max_stats[stat])
    ...
    def display_list(self):
        ...
        for i in range(1, min(self.visible_items, len(self.monsters))):
            ...
            pygame.draw.line(
                self.display_surface, COLORS["light-gray"], (left, y), (right, y)
            )
    ...
    def display_main(self, dt):
        ...
        ep_text = self.fonts["regular"].render(
            f"EP:{int(monster.energy)}/{int(monster.get_stat('max_energy'))}",
            False,
            COLORS["white"],
        )
        ...
        for index, (stat, value) in enumerate(monster_stats.items()):
            ...
            icon_rect = icon_surf.get_frect(
                midleft=single_stat_rect.midleft + vector(5, 0)
            )
            self.display_surface.blit(icon_surf, icon_rect)
        ...
        for index, ability in enumerate(monster.get_abilities()):
            ...
            pygame.draw.rect(
                self.display_surface, COLORS[element], text_rect.inflate(10, 10), 0, 4
            )
            self.display_surface.blit(text_surf, text_rect)
